...
def read_development_meta(meta_csv):
    ...
    df = pd.read_csv(meta_csv, sep = '\t|,',delimiter = "'", engine='python')
    df = pd.DataFrame(df)
    print(df.head())
    ...
    for row in df.iterrows():
        ...
        #augmentation = row[1]['augment']
        
        audio_names.append(audio_name)
        scene_labels.append(scene_label)
        #identifiers.append(identifier)
        source_labels.append(source_label)
        #augment.append(augmentation)
        
    return audio_names, scene_labels,  source_labels 
...
def calculate_features(args):
    ...
    # Paths
    #lst = ['none', 'pitch_1' , 'pitch_2', 'pitch_-1', 'pitch_-2', 'noise', 'speed_81', 'speed_107']
    #for x in lst:
    ...
    # Read meta csv
    if data_type == 'development':
        [audio_names, scene_labels, source_labels] = \
    ...
    for (n, audio_name) in enumerate(audio_names):
        ...
        hf['feature_1'].resize((n + 1, seq_len, mel_bins))
        hf['feature_1'][n] = feature
    ...
    if data_type == 'development':
        ...
        hf.create_dataset(name='source_label', 
                          ...
                          dtype='S20')
        #hf.create_dataset(name='augment', 
                         # data=[s.encode() for s in augment], 
                          #dtype='S20')                  
